...
//      Serial.println(testRH);
    ...
    POPS_RH_time=true;
    cy_inc=0;
    Ltime=(CT + offset)% 1000;
    Lsecs= CT/1000;
    Lsecs=Lsecs % 60;
    Serial.println("Ltime, Lsecs= " + String(Ltime) + " " + String(Lsecs));
  ...
  CycleMS=CT-LastCT;
//  if (cy_inc == 1)  {
//    Serial.println("At cy_int=1, CycleMS= " + String(CycleMS));
//  }
  cy_inc++;
  
  // the following was 400, now is 50
  if( PollTime && CycleMS > 50) {
    // Now Poll 
    PollTime=false;
...
LastPOPS= "POP=" + DataIn;
Serial.println("POPS arrives at: " + String(CycleMS));
}
